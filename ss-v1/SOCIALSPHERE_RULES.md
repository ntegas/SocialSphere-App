...
| 51 | Сборка падает: ресурс «does not exist / Access is denied» (напр. `mipmap-xxhdpi/ic_launcher_round.webp`) | НЕ баг кода — файл в ZIP валиден (страж **У53** проверяет целостность всех .webp: размер + сигнатура RIFF/WEBP). Причина — окружение Windows: неполная распаковка Проводником при длинном пути (файл числится в каталоге, но физически отсутствует/нулевой), блокировка антивирусом, или процесс держит файл. Лечение: распаковать 7-Zip в КОРОТКИЙ путь (`C:\ss\`), `gradlew clean` + `gradlew --no-build-cache assembleDebug`, папку — в исключения антивируса. OneDrive — лишь одна из причин, не единственная; не утверждать её без подтверждения |
