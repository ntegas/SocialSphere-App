...
object SearchEngine {
    ...
    private fun scoreContact(contact: Contact, q: String): SearchResult.ContactResult? {
        ...
        contact.messengers.forEach { m ->
            if (m.value.lowercase().contains(q)) { score += 60; matchField = m.type.label() }
        }
        ...
        contact.personalDetails.forEach { pd ->
            if (pd.value.lowercase().contains(q)) { score += 25; matchField = pd.category.label() }
        }
        // Gifts
        ...
        if (contact.relationshipType.label().lowercase().contains(q)) { score += 15; matchField = "Тип" }
    ...
    private fun scoreCompany(company: Company, q: String): SearchResult.CompanyResult? {
        ...
        if (company.industry.label().lowercase().contains(q)) { score += 60; matchField = "Индустрия" }
